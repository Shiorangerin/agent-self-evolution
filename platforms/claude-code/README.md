...
在 Claude Code 中接入自我进化系统：每次 agent 任务结束（`Stop` 事件）时，自动把本次会话轨迹归一化并交给采集器，满足条件时用 LLM 并行发起两路独立采集（失败互不影响）：技能候选写入候选区；用户画像草稿写入 `profiles/`。候选与画像草稿都不进入运行上下文，由用户手动触发「进化流程」审查启用 / 提炼进 USER.md。USER.md 由 install.sh 在全局 `~/.claude/CLAUDE.md` 中以 `@` 绝对路径引用注入（幂等）。
