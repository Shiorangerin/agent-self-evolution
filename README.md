...
- 你**手动说一句「总结一天的工作」或「进化」**，系统就会审查候选、启用好技能、维护长期记忆、沉淀经验教训教训；
- 日积月累，你的 agent 会越来越懂你的环境、你的工具链、你遇到过的每一个问题。
...
| 痛点 | 现状 | 本系统的解法 |
| --- | --- | --- |
| 同样的问题反复出现 | 每次排错都从零开始 | 经验与教训沉淀进 `LESSONS.md`，下次直接查证 |
...
```
        ...
        │   统计工具调用 / 错误修复 → 满足条件？                     │
        │   → 低成本 LLM 双路独立采集（失败互不影响）：              │
        │     ① 草拟 SKILL.md 候选  ② 提炼用户画像草稿             │
        └──────────────────────────┬──────────────────────────────┘
                                   ▼
        ┌─────────────────────────────────────────────────────────┐
        │   candidates/ 候选区 + profiles/ 画像草稿区               │
        │   （均未启用，不进上下文；增量采集：同一会话可多次，不丢）    │
        └──────────────────────────┬──────────────────────────────┘
                                   ▼  （你手动说一句「进化」）
        ┌─────────────────────────────────────────────────────────┐
        │   [进化流程] 先跑 evolve_brief.sh 一次探测（状态/候选/体检/草稿） │
        │   审查候选 → 格式校验 → 查重 → 价值评估                    │
        │   启用（软链）/ 并入现有 / 淘汰（记录原因）                 │
        │   技能体检（闲置/问题/优质三级）→ 归档 / 修复 / 保留      │
        │   提炼画像草稿 → USER.md；沉淀经验教训 → LESSONS.md → git 提交 │
        └──────────────────────────┬──────────────────────────────┘
                                   ▼
...
| `profiles/` | 用户画像草稿区：采集器提炼的持久画像条目（正向表述、禁举例、禁元信息），待进化流程融合进 USER.md |
| `skills/` | 已启用的技能源文件（Pi 上通过软链挂到技能加载目录生效） |
| `archived/` | 归档技能（从上下文消失但源文件保留） |
| `memory/USER.md` | 长期记忆：用户画像、偏好、习惯（进化流程从画像草稿融合提炼写入） |
| `memory/LESSONS.md` | 长期记忆：经验与教训、失败教训、规避方法 |
...
与技能候选同链路的双路采集：满足触发条件时，采集器在技能候选之外**独立发起一路 LLM 调用**（失败互不影响），从轨迹提炼「跨会话仍然成立」的持久画像草稿，写入 `profiles/<会话名>/`（`profile.md` 条目列表 + `meta.md` 来源信息）。

**采集提示词的硬性规范**（最小化原则，代码层机械校验拄住违规输出）：

- 只记录稳定偏好、工作习惯、硬性约束、长期背景；一次性内容不记录；
- 每条一句话直接陈述，**正向表述优先**，避免否定句式；
- **禁止任何举例**，禁止来源/时间/编号/注释等任何元信息；
- 全部条目总长 ≤600 字符（`SE_MAX_PROFILE_CHARS` 可调）；
- 无可提炼信息时输出 SKIP，不写盘。

**隐私防线（通用，不涉及任何具体用户信息）**：

- 轨迹摘要外发前统一脱敏：家目录绝对路径 → `~`；
- 隐私熔断：轨迹命中隐私路径模式（日记/环境变量/SSH 密钥/证书/凭证/钱包等通用词表，可用 `SE_PRIVATE_PATTERNS` 覆盖）时，该会话整场跳过采集，内容不外发给任何 LLM；宁可误杀不可放过，手动强制（`--force`）也不绕过。注意：`diary` 为宽匹配，整理日记类会话默认不产生任何候选/画像（符合隐私优先原则）。

草稿区堆积上限 `SE_MAX_PROFILES`（默认 20），进化流程把全部草稿融合提炼进 `memory/USER.md` 后归档，详见各平台进化流程说明书。

### 审查（Review）

进化流程第一步先跑 `scripts/evolve_brief.sh`，一次拿到状态、候选、体检与草稿全貌；然后对每个候选执行三步校验：

1. **格式校验**：`name`（小写字母数字连字符）、`description`（≤1024 字符、写明何时使用）、SKILL.md ≤8000 字符；可先用 `scripts/candidate_preflight.py` 零 token 机械预检；
2. **查重**：与已启用技能语义重复 → 并入或淘汰；
3. **价值评估**：会重复出现吗？步骤可复用吗？有真实注意点吗？

完整判定标准与处置命令见 `$SE_ROOT/docs/candidate-review.md`（安装后位于数据目录 `docs/`），流程按需读取，不占用常驻上下文。
...
| 文件 | 内容 | 注入方式 |
| --- | --- | --- |
| `USER.md` | 用户偏好、习惯、禁忌（采集器双路采集画像草稿，进化时融合提炼） | Pi 注入系统提示 / Claude Code 全局 CLAUDE.md `@` 引用 / Codex AGENTS.md 引用 |
| `LESSONS.md` | 经验与教训（场景/现象/根因/解法/来源） | 按需读取 |
...
这是最核心的价值。每次任务后，可复用的流程、遇到过的问题、总结出的解法都被结构化沉淀。半年后你的 agent 拥有一个**为你量身定制的私有技能库**，而不是每次从零推理。
...
进化流程定期做「技能体检」，分三级处置：
- **闲置技能**：≥15 天未使用且累计使用不足 10 次，或 ≥30 天未使用 → 由你决定归档或保留；
- **沉睡技能**：≥15 天未使用但累计使用 ≥10 次 → 只提示不归档，满 30 天仍未用才升级为归档候选；
- **问题技能**：失败 ≥2 次或失败率 >50% → 自动复核失败原因，能补注意点就补、步骤错了就修，屡败零胜建议淘汰；
- **优质技能**：多次使用且零失败 → 确认保留。

技能总量控制在 ~40 个以内，避免上下文被 description 淹没。

具体阈值、归因方法与归档/恢复/压缩命令见 `$SE_ROOT/docs/skill-health.md`。
...
- **每次**满足触发条件的任务（≥5 次工具调用或出现错误）都会产生**最多两次** LLM 调用（技能候选 + 用户画像各一次，失败互不影响），虽然成本经过控制（低推理强度 + 截断 + 输出限制），但**不是零成本**；
- 高频使用下（每天几十个任务），每天会产生几十次低成本调用（按两倍估算），每月累计可能达到数美元级别（取决于你的模型定价）；
