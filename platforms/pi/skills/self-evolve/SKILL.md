...
按记分卡分级处置：优秀与健康不做操作；未观测不处置；问题项复核失败归因，能修则修；闲置项（半月未用且低产，或满 30 天）列出后征求用户归档决定；沉睡项（半月未用但累计使用 ≥10 次）仅提示不归档；元技能与用户手动管理的技能仅提示或只读观察。
...
脚本先体检后写盘，体检不通过会中止且不改 state.json 与日志；改动范围含扩展或核心代码时自动跑全量体检，`--full` 可强制；数据根目录等于 git 仓库根时拒绝自动提交；全程持排他锁，并发运行会被挡住；先看计划用 `--dry-run`；不在 git 仓库内则跳过提交。提交信息缺省为 `self-evolve: <时间> 进化——<计数摘要>`，可用 `--message` 覆盖。
