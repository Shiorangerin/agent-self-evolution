...
- 经验与教训 → 追加到 `$SE_ROOT/memory/LESSONS.md`；
...
3. 超长技能（>8000 字符）压缩走省 token 规范：低成本模型起草（保留全部注意点/命令/参数，只删冗余）+ 人工审 diff 落盘；>8K 或涉隐私边界先征求用户意见。
...
- `@memory/LESSONS.md` — 经验与教训（`$SE_ROOT/memory/LESSONS.md`），按需读取；若相对路径无法解析，请改用 `$SE_ROOT/memory/LESSONS.md` 绝对路径。
